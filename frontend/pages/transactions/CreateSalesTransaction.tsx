...
import { Calculator, CreditCard, Banknote, FileCheck, Car } from 'lucide-react';
...
export function CreateSalesTransaction() {
  ...
  const createTransactionMutation = useMutation({
    ...
    onSuccess: (result) => {
      ...
      navigate(`/transactions/sales/${result.id}`);
    },
    onError: (error) => {
  ...
  const handleVehicleSelect = (vehicleId: string) => {
    ...
    if (vehicle?.approved_selling_price) {
      handleInputChange('vehicle_price', vehicle.approved_selling_price);
    }
  };
            ...
            <CardContent>
              <div className="space-y-4">
                <div className="space-y-2">
                  ...
                  >
                    <SelectTrigger>
                      <SelectValue placeholder="Choose a vehicle to sell" />
                    </SelectTrigger>
                    <SelectContent>
                      {vehicles?.vehicles.map((vehicle) => (
                        <SelectItem key={vehicle.id} value={vehicle.id.toString()}>
                          <div className="flex items-center justify-between w-full">
                            <span>{vehicle.vehicle_code} - {vehicle.brand} {vehicle.model} ({vehicle.year})</span>
            ...
            <div className="space-y-4">
              ...
              {formData.discount_amount > 0 && (
                <div className="flex justify-between text-lg text-green-600">
                  <span>Discount:</span>
                  <span className="font-mono">-${formData.discount_amount.toLocaleString()}</span>
                </div>
              )}
              ...
              <div className="flex justify-between text-2xl font-bold">
                <span>TOTAL:</span>
                <span className="font-mono text-green-600">${totalAmount.toLocaleString()}</span>
              </div>
